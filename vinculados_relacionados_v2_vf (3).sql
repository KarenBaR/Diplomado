...
relacionados_norm AS (
    SELECT
        ...
        CAST(tip_doc2 AS VARCHAR) AS tip_doc_vinculado,
        tip_vinculo,
...
reniec AS (
    SELECT
        ...
        est_civil,
        genero,
        CAST(tipdoc_conyugue AS VARCHAR) AS tipdoc_conyugue,
...
fallback_norm AS (
    ...
    JOIN reniec    rn ON rl.dni_hash_rrll = rn.dni_hash
    WHERE cr.num_ruc IS NULL                                   -- anti-join
...
rcc_empresa AS (
    SELECT
        a.codmes,
        CAST(a.num_ruc AS VARCHAR)  AS doc_hash_empresa,
        a.pct_normal_min,
        a.pct_cpp_min,
