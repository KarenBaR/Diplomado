...
def _cargar_raw() -> pd.DataFrame:
    ...
    try:
        df = pd.read_csv(BD_PATH, sep="|", dtype=str, encoding="latin-1")
        ...
        if "NOMBRE" in df.columns:
            df["NOMBRE"] = df["NOMBRE"].astype(str).str.strip().replace("nan", "")
...
class CalcularCEMRequest(BaseModel):
    ...
    cuotas_incluir: Optional[str] = Field(
        None,
        description=(
            "Componentes a sumar como GF estimado. Valores separados por coma: "
            "'rrll' = cuota_total_rrll, "
            "'empresa' = cuota_total_empresa, "
            "'conyugue' = cuota_total_conyugue. "
            "Ejemplo: 'rrll,empresa' (default) o 'rrll,empresa,conyugue'. "
            "Si se declara gasto_financiero_mensual, este campo se ignora."
        )
    )
...
class DatosLeadResponse(BaseModel):
    encontrado: bool
    ruc: str
    nombre: Optional[str] = None         # columna NOMBRE de la BD
    ...
    cuota_total_conyugue: Optional[float] = None      # cuota cónyuge
    cuota_sf_bd: Optional[float] = None               # cuota_sf = rrll + empresa
    cuota_sf_2_bd: Optional[float] = None             # cuota_sf_2 = rrll + empresa + cónyuge
    ...
    calificacion_cliente: Optional[str] = None    # calificacion_cliente_val
    calificacion_sbs: Optional[str] = None         # SBS_CALIFICACION
    saldo_col_ibk: Optional[float] = None          # saldo_mesact_col_sol — saldo IBK colocaciones
    max_atraso_ibk: Optional[int] = None           # max_atraso_ibk_coloc_directas_general (-1=sin dato)
    max_atraso_competencia: Optional[int] = None   # max_atraso_competencia_coloc_directas_ajustado_u12m
    max_atraso_sistema_6m: Optional[int] = None    # max_atraso_coloc_directas_general_u6m
    ...
    banco_3: Optional[str] = None
    saldo_banco_3: Optional[float] = None
    # Variables del modelo de propensión
    score_propension: Optional[float] = None
    grupo_propension_tda: Optional[str] = None
    grupo_propension_tv: Optional[str] = None
    flg_gestionado: Optional[bool] = None
    flg_acepta1: Optional[bool] = None
    flg_desembolsado: Optional[bool] = None
    recencia_pensara_meses: Optional[int] = None
    meses_gestionados_ult3m: Optional[int] = None
    monto_oferta_max_ult3m: Optional[float] = None
    deuda_var_pct_12m: Optional[float] = None
    deuda_var_pct_6m: Optional[float] = None
    deuda_var_pct_3m: Optional[float] = None
    max_atraso_ibk: Optional[float] = None
    max_atraso_competencia: Optional[float] = None
    cant_empresas_12m: Optional[float] = None
    meses_crecimiento_deuda: Optional[float] = None
    entidades_bancos_12m: Optional[float] = None
    ingreso_bruto_rrll: Optional[float] = None
    # Top 5 variables importancia del modelo (feature importance)
    v1_deuda_max_12m: Optional[float] = None          # 13.26% — Máx. Deuda SF Últ. 12M
    v2_var_deuda_noibk_12m: Optional[float] = None    # 8.45%  — Variación Deuda No-IBK Últ. 12M
    v3_saldo_vig_prestamos: Optional[float] = None    # 7.17%  — Prom. Saldo Vig. Préstamos Últ. 6M
    v4_nro_coloc_bancos: Optional[float] = None       # 5.99%  — Prom. Nro. Reg. Coloc. Bancos Últ. 12M
    v5_cant_empresas: Optional[float] = None          # 5.33%  — Cant. Empresas Promedio Últ. Año
...
def buscar_ruc(request: BuscarRucRequest):
    """
    PASO 1: Busca el RUC real del cliente en la BD (base_cem_v3).
    ...
    descripcion = str(lead.get("descripcion_val", "")).strip()
    nombre_col  = str(lead.get("NOMBRE", "")).strip()
    ...
    cuota_conyugue = float(lead.get("cuota_total_conyugue", 0) or 0)
    cuota_sf_1     = cuota_rrll + cuota_empresa                   # cuota_sf
    cuota_sf_2     = cuota_rrll + cuota_empresa + cuota_conyugue  # cuota_sf_2
    ...
    razon_social = nombre_col if nombre_col and nombre_col not in ("nan","N/D","") else giro
    ...
    # cuota_sf ya es la suma de cuota_total_rrll + cuota_total_empresa
    cuota_total_sf = cuota_rrll + cuota_empresa
    _linea_cuotas_det = (
    ...
    sbs_calif             = str(lead.get("SBS_CALIFICACION") or "").strip()
    saldo_col_ibk         = float(lead.get("saldo_mesact_col_sol") or 0)
    _atraso_ibk_raw       = float(lead.get("max_atraso_ibk_coloc_directas_general") or -1)
    _atraso_comp_raw      = float(lead.get("max_atraso_competencia_coloc_directas_ajustado_u12m") or -1)
    _atraso_sis_raw       = float(lead.get("max_atraso_coloc_directas_general_u6m") or -1)
    max_atraso_ibk        = int(_atraso_ibk_raw)  if _atraso_ibk_raw  > 0 else None
    max_atraso_competencia= int(_atraso_comp_raw) if _atraso_comp_raw > 0 else None
    max_atraso_sistema_6m = int(_atraso_sis_raw)  if _atraso_sis_raw  > 0 else None
    ...
    # ── Variables del modelo de propensión ──
    score_prop  = float(lead.get("score_propension") or 0)
    # Top 5 feature importances
    v1 = float(lead.get("deuda_total_max_12m") or 0)
    v2 = float(lead.get("deuda_noibk_var_pct_12m") or 0)
    v3 = float(lead.get("avg_saldo_vig_prestamos_general_amt_u6m") or 0)
    v4 = float(lead.get("avg_nro_reg_coloc_directas_bancos_ajustado_u12m") or 0)
    v5 = float(lead.get("cant_empresas_promedio_12m") or 0)
    grp_tda     = str(lead.get("grupo_propension_tda_f_v2") or "").strip()
    grp_tv      = str(lead.get("grupo_propension_tv_v2") or "").strip()
    flg_gest    = str(lead.get("flg_gestionado") or "0") == "1"
    flg_acep    = str(lead.get("flg_acepta1") or "0") == "1"
    flg_desemb  = str(lead.get("flg_desembolsado") or "0") == "1"
    rec_meses   = int(float(lead.get("recencia_pensara_meses") or 0))
    meses_gest  = int(float(lead.get("meses_gestionados_ult3m") or 0))
    oferta_max  = float(lead.get("monto_oferta_max_ult3m") or 0)
    dvar_12m    = float(lead.get("deuda_total_var_pct_12m") or 0)
    dvar_6m     = float(lead.get("deuda_total_var_pct_6m") or 0)
    dvar_3m     = float(lead.get("deuda_total_var_pct_3m") or 0)
    # Filtrar variaciones absurdas (sin deuda previa → -9999999999)
    dvar_12m = dvar_12m if dvar_12m is not None and -500 <= dvar_12m <= 1000 else None
    dvar_6m  = dvar_6m  if dvar_6m  is not None and -500 <= dvar_6m  <= 1000 else None
    dvar_3m  = dvar_3m  if dvar_3m  is not None and -500 <= dvar_3m  <= 1000 else None
    atr_ibk     = float(lead.get("max_atraso_ibk_coloc_directas_general") or -1)
    atr_comp    = float(lead.get("max_atraso_competencia_coloc_directas_ajustado_u12m") or 0)
    cant_emp    = float(lead.get("cant_empresas_promedio_12m") or 0)
    meses_crec  = float(lead.get("deuda_total_meses_crecimiento_12m") or 0)
    ent_bancos  = float(lead.get("avg_nro_entidades_bancos_u12m") or 0)
    ing_rrll    = float(lead.get("ingreso_bruto_total_rrll") or 0)
    ...
    return DatosLeadResponse(
        encontrado=True,
        ruc=ruc,
        nombre=nombre_col if nombre_col and nombre_col not in ("nan","N/D","") else None,
        razon_social=razon_social,
        giro_canonico=giro,
        subsector=subsector,
        margen_asignado=margen,
        deuda_total_sbs=deuda,
        pago_mensual_financiero=cuota_sf_1,
        cuota_total_rrll=cuota_rrll,
        cuota_total_empresa=cuota_empresa,
        cuota_total_conyugue=cuota_conyugue if cuota_conyugue > 0 else None,
        cuota_sf_bd=cuota_sf_1 if cuota_sf_1 > 0 else None,
        cuota_sf_2_bd=cuota_sf_2 if cuota_sf_2 > 0 else None,
        ...
        calificacion_sbs=sbs_calif or calif_cred or None,
        saldo_col_ibk=saldo_col_ibk if saldo_col_ibk > 0 else None,
        max_atraso_ibk=max_atraso_ibk,
        max_atraso_competencia=max_atraso_competencia,
        max_atraso_sistema_6m=max_atraso_sistema_6m,
        ...
        banco_3=str(lead.get("nomempresafinanc_desc_3") or "").strip() or None,
        saldo_banco_3=float(lead.get("saldo_ajustado_ent_3") or 0) or None,
        score_propension=score_prop if score_prop > 0 else None,
        v1_deuda_max_12m=v1 if v1 > 0 else None,
        v2_var_deuda_noibk_12m=v2 or None,
        v3_saldo_vig_prestamos=v3 if v3 > 0 else None,
        v4_nro_coloc_bancos=v4 if v4 > 0 else None,
        v5_cant_empresas=v5 if v5 > 0 else None,
        grupo_propension_tda=grp_tda or None,
        grupo_propension_tv=grp_tv or None,
        flg_gestionado=flg_gest,
        flg_acepta1=flg_acep,
        flg_desembolsado=flg_desemb,
        recencia_pensara_meses=rec_meses if rec_meses > 0 else None,
        meses_gestionados_ult3m=meses_gest,
        monto_oferta_max_ult3m=oferta_max if oferta_max > 0 else None,
        deuda_var_pct_12m=dvar_12m or None,
        deuda_var_pct_6m=dvar_6m or None,
        deuda_var_pct_3m=dvar_3m or None,
        cant_empresas_12m=cant_emp if cant_emp > 0 else None,
        meses_crecimiento_deuda=meses_crec if meses_crec > 0 else None,
        entidades_bancos_12m=ent_bancos if ent_bancos > 0 else None,
        ingreso_bruto_rrll=ing_rrll if ing_rrll > 0 else None,
...
def calcular_cem(request: CalcularCEMRequest):
    ...
    if lead:
        nombre_col   = str(lead.get("NOMBRE", "") or lead.get("nombre", "")).strip()
        descripcion  = str(lead.get("descripcion_val", "")).strip()
        giro         = str(lead.get("giro_canonico_match", "")).strip()
        razon_social = nombre_col if nombre_col and nombre_col not in ("nan","N/D","") else giro
        margen       = float(lead.get("margen_asignado", 0.30))
        deuda_sbs    = float(lead.get("deuda_total_max_12m", 0))
        # Cuotas SF de la BD
        _cuota_rrll    = float(lead.get("cuota_total_rrll", 0) or 0)
        _cuota_empresa = float(lead.get("cuota_total_empresa", 0) or 0)
        _cuota_cony    = float(lead.get("cuota_total_conyugue", 0) or 0)
        pago_fin_bd    = _cuota_rrll + _cuota_empresa                  # cuota_sf
        pago_fin_bd_2  = _cuota_rrll + _cuota_empresa + _cuota_cony    # cuota_sf_2
        analisis       = construir_analisis_riesgo(lead)
    else:
    ...
    # 3. Gastos financieros — suma de componentes seleccionados
    if request.gasto_financiero_mensual is not None:
        # El ejecutivo declaró el GF real — siempre tiene prioridad
        gasto_fin = request.gasto_financiero_mensual
        fuente_gf = "declarado_ejecutivo"
    else:
        # Determinar componentes a sumar según cuotas_incluir
        componentes = [c.strip().lower() for c in (request.cuotas_incluir or "rrll,empresa").split(",") if c.strip()]
        if not componentes:
            componentes = ["rrll", "empresa"]
        gasto_fin = 0.0
        partes = []
        if "rrll"     in componentes and _cuota_rrll    > 0: gasto_fin += _cuota_rrll;    partes.append(f"rrll={_cuota_rrll:.2f}")
        if "empresa"  in componentes and _cuota_empresa > 0: gasto_fin += _cuota_empresa; partes.append(f"empresa={_cuota_empresa:.2f}")
        if "conyugue" in componentes and _cuota_cony    > 0: gasto_fin += _cuota_cony;    partes.append(f"conyugue={_cuota_cony:.2f}")
        if gasto_fin == 0 and deuda_sbs > 0:
            gasto_fin = round(deuda_sbs * TEA / 12, 2)
            fuente_gf = "estimado_tea"
        else:
            fuente_gf = "cuotas_bd:"+(",".join(componentes) if partes else "sin_datos")
    ...
    detalle["sector_inferido"] = sector
    detalle["fuente_gf"]       = fuente_gf
    detalle["cuotas_incluir"]  = request.cuotas_incluir or "rrll,empresa"
    detalle["cuota_sf_bd"]     = round(pago_fin_bd, 2)
    detalle["cuota_sf_2_bd"]   = round(pago_fin_bd_2, 2)
