# =============================================================================
# Orquestador SALDOS IBK  -  version corregida
# Cambios respecto al notebook original:
#   1) SQL_UNIVERSO: el universo nace de los CLIENTES CON SALDO (CUC -> RUC),
#      no del maestro completo. Elimina RUC fantasma sin perder churners.
#   2) SQL_PIVOT: se quito el filtro  AND situacion_cartera_dsc = 'VIGENTE'
#      del WHERE de base_saldos_raw. Ese filtro borraba todo el pasivo y el
#      deterioro (vencido/castigado), dejando las variables en cero.
#      La logica de VIGENTE ya vive dentro de cada CASE (situacion_cartera_cd).
#
# Uso: edita el bloque CONFIG (abajo) y ejecuta el script completo.
# =============================================================================



# 1) Instalacion de dependencias
#    (si ya tienes awswrangler instalado en el kernel, puedes borrar este bloque)
import subprocess, sys
subprocess.run([sys.executable, "-m", "pip", "install", "-q", "awswrangler"], check=False)
...
PERIODO_HASTA = 202606      # ultimo mes a generar  (YYYYMM)
...
def run_ddl(sql: str, label: str = ""):
    """Ejecuta un DDL/CTAS en Athena (workgroup de IBK) y espera a que termine."""
...
# 4a) Template SQL - UNIVERSO MAESTRO (anclado en clientes con saldo)
SQL_UNIVERSO = r"""
CREATE TABLE disc_comercial.HM_UNIVERSO_MAESTRO_SALDOS
WITH (
    format = 'Parquet',
    parquet_compression = 'SNAPPY'
)
AS (
    -- Paso 1: CUCs que tuvieron algun saldo en la ventana
    WITH clientes_con_saldo AS (
        SELECT DISTINCT CAST(codigo_unico_cliente_cd AS VARCHAR) AS cuc
        FROM e_perm_aws.t_agg_vpc_saldos_diarios
        WHERE periodo_val BETWEEN {uni_inicio} AND {uni_fin}
    ),
    -- Paso 2: equivalencia CUC -> RUC
    equivalencia_cuc_ruc AS (
        SELECT DISTINCT
            CAST(cuc_num AS VARCHAR) AS cuc,
            num_ruc_val              AS num_ruc
        FROM e_perm_aws.T_VPC_CLIENTE_BANCA_FINAL_HST
        WHERE num_ruc_val IS NOT NULL
    )
    -- Paso 3: RUC de esos clientes
    SELECT DISTINCT e.num_ruc
    FROM clientes_con_saldo c
    JOIN equivalencia_cuc_ruc e ON c.cuc = e.cuc
)
"""


# 4b) Template SQL - PIVOT (M0 + lags, ventana de 12 meses)
SQL_PIVOT = r"""
CREATE TABLE disc_comercial.HM_SALDO_VPC_IBK_PIVOT
WITH ( 
    format = 'Parquet', 
    parquet_compression = 'SNAPPY', 
...
base_saldos_raw AS (
    SELECT
        ...
        SUM(CASE WHEN aa.tipo_prod_dsc IN ('DEPÓSITOS')    THEN saldo_actual_sol_mto    ELSE 0 END) AS saldo_punta_pasivo,
        SUM(CASE WHEN aa.tipo_prod_dsc IN ('DEPÓSITOS')    THEN saldo_promedio_sol_mto  ELSE 0 END) AS saldo_promedio_pasivo,
...
# 4c) Template SQL - FEATURES (derivadas del mes)
SQL_FEATURES = r"""
CREATE TABLE disc_comercial.HM_SALDO_VPC_IBK_FEATURES_{periodo_objetivo}
WITH ( 
    format = 'Parquet', 
    parquet_compression = 'SNAPPY', 
    partitioned_by = ARRAY['PERIODO_2']
